                ...
                resolve: async ({ importerPath, importPath }) => {
                    ...
                    return await task.do(`Resolving: ${importPath}`, async task => {
                        ...
                        try {
                            ...
                            task.log(`Package ${string.name} subpath ${string.path} version ${string.version}`);
                            if (!string.version) {
                                if (importerPath in options.files) {
                                    string.version = options.files[importerPath].dependencyTree[string.name]?.version;
                                } else {
                                    const importer = JsDelivr.parseUrl(importerPath);
                                    string.version = importer.name !== string.name ? NPM.resolveDependencyVersion(dependencyTree, importer.name, importer.version, string.name) : importer.version;
                                }
                                if (!string.version) {
                                    const polyfill = NODE_BUILTIN_POLYFILLS[string.name];
                                    if (polyfill) {
                                        const version = dependencyTree[polyfill.name]?.version ?? dependencyTree[string.name]?.version ?? polyfill.version;
                                        task.log(`Resolving Node.js built-in "${string.name}" via browser polyfill "${polyfill.name}@${version}"`);
                                        string.name = polyfill.name;
                                        string.version = version;
                                    } else if (importerPath in options.files) {
                                        throw new Error(`Unable to determine package "${string.name}" version.`);
                                    } else {
                                        task.log(`Returning mock module for package "${string.name}" as import doesn't have it as a dependency.`, { level: 'warning' });
                                        return;
                                    }
                                }
                            }
                            const filePath = await JsDelivr.resolveImportFilePath(string.name, string.version, string.path);
                            task.log(`Import resolved to file path "${filePath}"`);
                            const file = await JsDelivr.getFile(string.name, string.version, filePath);
                            task.log('Returning file $[content] from CDN', { attachments: { content: { type: 'text', value: file.content } } });
