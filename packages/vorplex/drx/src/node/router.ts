...
export const DrxRouter = {
    from(parent: Element): DrxRouter | undefined {
        const element = parent.querySelector(`:scope > ${NodeType.Router}`);
        return element ? DrxRouter.parse(element) : undefined;
    },
    parse(element: Element): DrxRouter {
        const routes: Record<string, string> = {};
        for (const node of DrxDom.getNodes(element, NodeType.RouterRoute)) {
            const route = DrxDom.getRequiredAttribute(node, 'route');
            const page = DrxDom.getRequiredAttribute(node, 'page');
            routes[route] = page;
        }
        return {
            routes
    ...
    mount(container: Node, router: DrxRouter, context: RenderContext): void {
        ...
        Signal.effect(() => {
            ...
            DrxPage.mount(host, page, routedContext);
        });
        Signal.cleanup(() => {
