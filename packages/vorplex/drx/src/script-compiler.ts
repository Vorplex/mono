...
// Identity only — exists so scripts can write `DRX.defineApp(drx => class {...})` naturally.
...
export const ScriptCompiler = {
    ...
    async compile(state: DrxDocumentState): Promise<CompiledScripts> {
    ...
    // Evaluates the combined bundle fresh per call (synchronous) — scripts only ever see `DRX` as a bare
    // global; page/component state is only reachable via the `drx` parameter passed to the factory.
    instantiate(compiled: CompiledScripts, id: string | undefined, drx: any): any {
        if (!id || !compiled.keys.has(id)) return undefined;
        const module = ModuleLoader.evaluate(compiled.bundle, { DRX: ScriptRuntimeApi });
        const factory = module?.[compiled.keys.get(id)!];
        return factory ? factory(drx) : undefined;
    },
    ...
    // Creates a fresh drx.services surface for a scope's own declared services, sharing the supplied
    ...
    instantiateServices(serviceIds: string[], state: DrxDocumentState, compiled: CompiledScripts, instances = new Map<string, any>()): Record<string, any> {
