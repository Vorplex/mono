...
DRX is a declarative markup language for reactive web apps, built from composable nodes (`<x-*>` tags) that compile to a runtime powered by Vorplex.
...
```html
<script src="https://cdn.jsdelivr.net/gh/Vorplex/mono@shtml/packages/vorplex/drx/standalone/cdn/drx.js"></script>
<x-app>
  <x-page name="home">
...
| ------------ | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<x-router>` |         | Holds `<x-route>` entries. Once mounted, any rendered page can read the current match reactively — `router.route()` / `router.params.<n>()` in a template, or `drx.router.route` / `drx.router.params` (plain reads) and `drx.router.navigate('/posts')` from a script. |
| `<x-route>`  |         | Declares one path → page mapping inside `<x-router>`                                                                                                                                                                                                                    |
|              | `route` | The path to match, e.g. `/posts/{id}`                                                                                                                                                                                                                                   |
|              | `page`  | The `<x-page>` name to render                                                                                                                                                                                                                                           |
...
<x-app>
  <x-router>
    ...
    <x-route route="/404" page="404"></x-route>
  </x-router>
...
Declares a page, referenced by `<x-route>`, `<x-page-container>`, or `drx.pages.<name>`.
...
| `<x-property>`           |              | Declares a prop: an entry point. Exposed as a callable signal in the template (`level()`); read-only from the component's own script (`drx.component.props.level()`).                                                                    |
|                          | `name`       | The prop's identifier                                                                                                                                                                                                                    |
|                          | `type?`      | TSON type; defaults to `any`                                                                                                                                                                                                             |
| `<x-event>`              |              | Declares an event: an exit point. Call `drx.component.events.<name>.emit(value)` from the component's script to fire it.                                                                                                                 |
...
| `type?` | MIME type for an inline asset; omitting it falls back to `application/octet-stream` |
...
| Node            | Name        | Description                                                                                                                                                                                                                                                              |
| --------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `<x-api>`       |             | Declares an API client, reached at runtime via `drx.apis.<name>.<endpoint>.request({ parameters, headers, body })`.                                                                                                                                                      |
  ...
  (drx) =>
    class {
      async onMount() {
        ...
        await drx.apis.todoApi.create.request({ body: { title: "Ship it" } });

        await drx.apis.todoApi.remove.request({
...
A plain class for behavior that doesn't belong to any one page or component — no template, just a script. Reached at runtime via `drx.services.<name>.<method>(...)`.
...
Every app/page/component/service script follows the same shape: `export default DRX.defineApp/definePage/defineComponent/defineService(drx => class { ... })`. Inline, it needs `<script type="application/typescript">`; imported as a `.ts` file, the same `export default` line stands alone.

In a script, state is read through `drx`: `drx.page.variables.<n>.get()/set(value)/reset()/validate()` (same surface for `drx.app.variables.<n>`). Pages inherit the app: a page script can call `drx.app.instance.<method>(...)` and read/write `drx.app.variables.<n>` directly. Without a router, `drx.pages.<n>.show()` switches which page is displayed.
...
<x-page name="counter">
  ...
  <script type="application/typescript">
    export default DRX.definePage(drx => class {
        onMount() {
            console.log('mounted with', drx.page.variables.count.get());
        }
        reset() {
            drx.page.variables.count.reset();
        }
    });
  </script>
...
Every `type` attribute (on `<x-variable>`, `<x-property>`, `<x-event>`, `<x-body>`, `<x-response>`) accepts one of these primitive names, or an `<x-type>` name — this is TSON, DRX's schema notation:
...
<x-page name="confirm-delete">
  <script type="application/typescript">
    export default DRX.definePage(drx => class {
        confirm() {
            drx.modal.close({ confirmed: true });
        }
    });
  </script>
  ...
  (drx) =>
    class {
      async openDeleteConfirm() {
        const result = await drx.pages["confirm-delete"].showModal({
          data: { name: "Q3 Report" },
        });
        if (result.confirmed) {
          /* ... */
...
`options.data` becomes `drx.modal.data` (script) / `modal.data` (template), available from `onMount()` onward. The framework supplies only a bare, full-screen host — the modal page owns its own backdrop, centering, and sizing.

## Gotcha: Table Layouts

DRX source is parsed with the browser's own HTML parser, so `<table>`/`<tbody>`/`<tr>`'s restricted content model applies to `<x-if>`/`<x-for>`/`<x-import>` exactly as it would to any other unrecognized element — they get silently relocated out of the row/cell structure they were meant to wrap:
