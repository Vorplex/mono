...
function ensureSheetLoaded(): Promise<void> {
    loading ??= fetch(SHEET_URL)
        ...
        .then(text => {
            const sheet = new DOMParser().parseFromString(text, 'image/svg+xml').documentElement;
            sheet.setAttribute('style', 'position: absolute; width: 0; height: 0; overflow: hidden;');
